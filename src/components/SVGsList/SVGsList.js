  ...
  if(feature.children) {
    ...
    return (
      <div>
        ...
        <ul className='vectors-list' style={{height: "calc(100vh - 255px)"}}>
          ...
          {
            svgIds.map(x=><li key={x} className={`item ${x===feature.typeSelected?'selected':''}`} onClick={()=>changeChildTypeSelected(x)}>
              <svg xmlns='http://www.w3.org/2000/svg' style={{stroke: "#b4bedb"}}  width="100%" height="100%" viewBox="0 0 200 200">
                <use xlinkHref={`#${x}`} />
              </svg>
            </li>)
