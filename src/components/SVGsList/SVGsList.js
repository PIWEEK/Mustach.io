    ...
    return (
      <div>
        ...
        <ul className='vectors-list'>
          ...
          <svg xmlns='http://www.w3.org/2000/svg' style={{stroke: "#b4bedb"}} width="100%" height="100%" viewBox="0 0 360 360">
          ...
          </li>
          {
            svgIds.map((x, index)=><li key={x} className={`item ${x===feature[`${subsectionSelected.toLowerCase()}Type`] ? 'selected' : ''}`}  onClick={()=>changeTypeSelected(++index)}>
              <svg xmlns='http://www.w3.org/2000/svg' className={feature.subSectionsSelected} style={{stroke: "#b4bedb"}}  width="100%" height="100%" viewBox="0 0 360 360">
                <use xlinkHref={`#${x}`} />
              </svg>
            </li>)
          }
        ...
        <ul className='vectors-list' style={{/*height: "calc(100vh - 220px)"*/}}>
          ...
          </li>
          {
            svgIds.map((x, index)=><li key={x} className={`item ${x===feature.typeSelected?'selected':''}`} onClick={()=>changeTypeSelected(++index)}>
              <svg xmlns='http://www.w3.org/2000/svg' className={feature.id} style={{stroke: "#b4bedb"}}  width="100%" height="100%" viewBox="0 0 360 360">
                <use xlinkHref={`#${x}`} />
              </svg>
            </li>)
          }
